...
async def student_calendar(
    ...
    # Get the relevant student
    if user.role == "student":
        student = db.query(StudentProfile).filter(StudentProfile.user_id == user.id).first()
    else:
        # For parent, get first linked student (demo)
        student = db.query(StudentProfile).first()
    ...
    # Practice logs
    for log in db.query(PracticeLog).filter(PracticeLog.student_id == student.id).all():
        if log.date not in calendar_data:
            calendar_data[log.date] = {"practice": [], "assignments": [], "parent_activity": []}
        calendar_data[log.date]["practice"].append({
            "minutes": log.minutes,
            "notes": log.notes or ""
        })

    # Assignments (use week_start as approximate date)
    for assignment in db.query(Assignment).filter(Assignment.student_id == student.id).all():
        date_key = assignment.week_start
        if date_key not in calendar_data:
            calendar_data[date_key] = {"practice": [], "assignments": [], "parent_activity": []}
        calendar_data[date_key]["assignments"].append({
            "title": assignment.title,
            "description": assignment.description
        })

    # Parent messages/uploads as activity
    messages = db.query(Message).filter(Message.student_id == student.id).all()
    for msg in messages:
        date_key = msg.created_at.strftime("%Y-%m-%d")
        if date_key not in calendar_data:
            calendar_data[date_key] = {"practice": [], "assignments": [], "parent_activity": []}
        calendar_data[date_key]["parent_activity"].append({
